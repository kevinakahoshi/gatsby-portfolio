...
const ApplicationsSkeletonDescriptionStyles = styled.div`
  ...
  grid-template-rows: repeat(
    ${({ numberOfRows }) => numberOfRows || 3},
    1.35rem
  );
  grid-gap: 0.25rem;

  ${({ groupSelectorString }) =>
    groupSelectorString || '.row-1-bar, .row-2-bar, .row-3-bar'} {
    width: 100%;
...
const ApplicationsSkeletonDescription = ({ numberOfRows = 0 }) => {
  ...
  const rowElements = rowClasses.map((name) => (
    <div key={name} className={name} />
  ));

  const groupSelectorString = rowClasses.map((name) => `.${name}`).join(', ');
